...
def rock_cleaner(df): 
    ...
    df = df.loc[(df.allelectrons_Total > 1.0)] # Removes both non-existent values from the low end, and the upper 1% of data that may be outliers
    df = df.loc[(df.allelectrons_Total < 725)] #Changed from having both parameters in the same line so I can comment out upper (outlier)
    df = df.loc[(df.density_Total > 0.1)] # Removes those values with spurious and low densities, including 0, and removes the few with excessively high density, likely outliers
    df = df.loc[(df.density_Total < 200)] # Again, placed in separate line to comment out for outlier removal
